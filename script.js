...
// --- UPDATED CHATBOT FUNCTIONALITY TO CONNECT TO BACKEND ---

// 1. LISTENERS: These tell the browser to watch for clicks or Enter keys
...
// 2. THE MAIN FUNCTION: This sends the message to your Python Backend
async function handleChatbotMessage() {
    const input = document.getElementById('chat-input');
    const message = input.value.trim(); 
    if (message === '') return;

    // Show your message in the chat box
    ...
    loadingEl.className = 'text-xs text-gray-500 italic mb-4';
    loadingEl.textContent = "Bot is typing...";
    ...
    try {
        ...
        // Remove loading and show bot's answer
        ...
        addMessageToChat('bot', "Server is offline. Please run main.py.");
    }
}

// 3. UI FUNCTION: This creates the chat bubbles
function addMessageToChat(sender, message) {
    const chatBox = document.getElementById('chat-box');
    const messageEl = document.createElement('div');
    messageEl.className = sender === 'user' ? 'flex justify-end mb-4' : 'flex items-start mb-4';
    
    if (sender === 'user') {
        messageEl.innerHTML = `<div class="p-3 bg-blue-600 text-white rounded-xl max-w-[80%]">${message}</div>`;
    } else {
        messageEl.innerHTML = `
            <div class="w-8 h-8 rounded-full bg-blue-500 flex items-center justify-center text-white text-xs mr-2">Bot</div>
            <div class="p-3 bg-gray-100 rounded-xl max-w-[80%] text-gray-800">${message}</div>`;
    }
...
function openReal3DModel() {
    window.open(
        "https://autode.sk/4oXlmwB",
        "_blank",
        "noopener,noreferrer"
    );
}
